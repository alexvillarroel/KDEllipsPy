...
# Decimation by 4 along x-axis easy:
i = 0
j = 0
size_slip = int(slip_size/4)
B = np.zeros(size_slip)
for i in range(size_slip):
   B[i] = (A[j]+A[j+1]+A[j+2]+A[j+3])/4.
   j +=4

# Decimation along y-axis clever:
out_slip = open("../Evento/slip_red.res",'w')
size_slip = int(slip_size/16)
slip = np.zeros(size_slip)
j=0
for i in range(size_slip):
   slip[i] = (B[j]+B[j+nx]+B[j+2*nx]+B[j+3*nx])/4.
   out_slip.write("%g\n"%slip[i])
   j +=1
   if(i%nx==0):j = (i*4)
print("Slip contains now", len(slip), "elements")
out_slip.close()

# Identify the number of existent momentos.dat files and retrieve the 
# minimum misfits of each of them:
m_files = glob.glob("../Evento/momentos*.dat")
misfits = []
for i in range(len(m_files)):
   moments = np.loadtxt("../Evento/momentos%d.dat" % (i))
   min_misfit = min(moments[:,1])
   misfits.append(min_misfit)

# Identify the minimum misfit among all momentos.dat files, and retrieve
# the slip of its associated model:
pos_slip = np.where(misfits == min(misfits))[0]
selected_moments = np.loadtxt("../Evento/momentos%d.dat" % (pos_slip[0]))
pos_best_slip = np.where(selected_moments[:,1] == min(selected_moments[:,1]))[0]
best_slip = selected_moments[pos_best_slip,2]
if len(best_slip) > 1:
   best_slip = max(best_slip)

# Calculate Mo and Mw of the best model:
Mo = mu*best_slip*((dx*1000)*(dx*1000))
Mw = (np.log10(Mo)-9.1)/1.5
print("Mo = %0.4e (Nm)" % (Mo))
print("Mw = %0.2f" % (Mw))

# Read axi.hist file and retrieve strike, dip and rake:
hist = np.loadtxt("../Evento/axi.hist")
strike = hist[1,2]
dip = hist[1,3]
rake = hist[1,4]

# Retrieve the maximum slip on the fault:
hist_max = max(slip[0:size_fault])
hist_min = min(slip[0:size_fault])
...
for name in nombres.readlines():
   print(i+1, name[0:-1])
   if(name==''): continue
   plt.text(estaciones[i,1]+0.5,estaciones[i,0]-0.5,name)
   i+=1
   if(i>=nstac): break
# Plot limits:
