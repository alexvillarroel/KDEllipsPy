...
c
c*************************************************************************
c       Modif S. Peyrat for dynamic inversion
c
c       some format 
c       some output
c       restart
...
c      write(*,*)' I am node ',iproc,' lroot = ',lroot
       write(*,*)' node ',iproc,' lroot = ',lroot,' name = ',hostname


c    Call user specific setup for forward modelling
c
      call user_init(nd,range,scales,nd1,nd2,iproc)
c      write(*,*)' I am node ',iproc,' return from nd = ',nd
c                                    
...
c
      if(lroot)then
           ...
           open(lu_misf, file='misfits.dat', status='unknown')
           open(lu_modls, file='models.dat', status='unknown')
      endif
...
c
     ...
     &        istype,monte,calcmovement,scales,misfit,noforward)
c
...
c
      do 20 it = 1,itmax+1
c  skip forward modelling for starting models if they have been read in 
c                                    from a NAD file
...
c     Get current model
c                                    and execute the direct problem
c
               ii = 1+(i-1+ntot)*nd

               call transform2raw
     &             (na_models(ii),nd,range,scales,na_model)
               call forward(nd, na_model, misfitval,nd1,nd2,iproc)
c               print *,' proc=',iproc,'i = ',i,
c     &               ' ii=',ii,' misfval=',misfitval
c            write(*,'(2g12.5)')(na_model(j), na_models(ii+j),j=1,nd)
c
               jj = ntot + i
               misfit(jj) = misfitval
c
 ...
 31       continue
          ...
          if(lroot)then
               write(*,*)'cycle, ntot=',it, ntot
c               write(*,*)'ns,nd =',ns, nd
               write(lu_misf,'(1g12.5)')(misfit(ntot+i),i=1,ns)
               
c               write(*,*)(misfit(ntot+i),i=1,ns)
c               write(*,*)ns,' models :'
               do i=1,ns
c                  write(*,*)'misfit ',i,' = ', misfit(ntot+i)
                  ii = (i-1+ntot)*nd
                 ii=ii+1
                 call transform2raw
     &             (na_models(ii),nd,range,scales,na_model)
c                 write(*,'(8g12.5)')(na_model(j),j=1,nd)
          
                 write(lu_modls,'(1g12.5)')(na_model(j),j=1,nd)
               enddo
           endif      
           t1 = cputime(t2,t3)
           tmpi = tmpi + t2
...
c                                    call user supplied I/O routine
     ...
     &        nsamplei, nsample, itmax, nh_max, nh, header,nd1,nd2)
c
c
...
c                                               write direct access nad file
     ...
     &       (lu_nad,fnme,nd,ntot,
c     &        nh,nh_user,header,1,na_models,misfit)
     &        nh,nh_user,header,0,na_models,misfit)
...
c
c     Question and answer session  to read in NA options.
      do i=1,3
         read(lu_na,*)
      end do
c
...
c     
      ...
      if(sobol)then
c    Initialize 1-D Sobol sequence to control initial dimension 
c                                    in NA random walk
...
c   generate coefficients for quasi-random 
...
c
 ...
 100    format(3x,i4,2x,5(e14.6,1x))
...
c       Modif Sophie  for restart part

c           ATTENTION!!! NOT FINISHED
c          this is done for the restart, unless that
c          we don't have the same values, because ran3 must be called
c          the same time than old inversion
c                                               ATTENTION!!!
c                                               this is done only for 
c                                               pseudo-random part and not 
c                                               for quasi-random
                 do j=1,nd
                    a = ran3(iseed)
                 end do
...
c         open(lu_nad,file='models.in',status='old',err=33)
c         k = 1
c 32         read(lu_nad,*,end=34,err=33)
c            read(lu_nad,*,err=33)misfit(k)
c           read(lu_nad,*)(na_models(i,k),i=1,nd)

c                                               Modif Sophie to read
c                                               misfits.dat and models.dat
c                                               to restart if there is
c                                               no na.nad
c                                               end Modif sophie
...
c      ATTENTION!!! this is done 
c                                               for the restart, unless that
c                                               we don't have the same values,
c                                               because ran3 must be called
c                                               the same time than old 
c                                               inversion
c                                               ATTENTION!!!
c                                               this is done only for 
c                                               pseudo-random part and not 
...
c        Generate initial uniform random sample 
...
c          read(10,*)mdeg,ip,num
      do mdeg=5,10
           if(mdeg.gt.maxdeg)print *, 'too many degrees'
...
c                                       quasi-deviate
        if(mode.eq.0)then
           ...
           do 15 j=1,MAXBIT
             ...
             im=im/2
15         continue
           print *, 'MAXBIT too small in sobol'
1          im=(j-1)*MAXSEQ
...
c         write(50,*)' n ',n,' i',i,' j',j,' dev ',dev(i,j)
         ...
         ran = ran3(iseed)
           deviate = x1 + (x2-x1)*ran
c          write(*,*)'NA_deviate'
c         write(*,*)'iseed=',iseed,' ran=',ran,' deviate=',deviate
...
c         write(*,*)'irandomvalue'
c         write(*,*)'iseed=',iseed,' rran=',rran,' iran=',iran
        end if
...
c                                    write out optimum model
      ...
      do i=1,nd
c         write(lu_det,*)' ',i,' : ',model_opt(i)
            write(lu_det,*)model_opt(i)
      end do
        write(lu_det,100)
