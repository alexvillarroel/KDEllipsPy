...
c						forward modelling
c
	call user_init(nd,range,scales,nd1,nd2)
c						
...
c
           if(summary)then
              write(lu_out,*)
              write(lu_out,*)' start forward modeling iteration: ',it-1
	   end if
...
c						from a NAD file
	   ...
	   if(it.eq.1.and.noforward)go to 31
           t1 = cputime(t2,t3)
c
c  Boucle sur les NS modèles de l-échatillon    
c
	   do 30 i = 1,ns
...
c
     ...
     &             (na_models(ii),nd,range,scales,na_model)
	      ...
	      call forward(nd, na_model, misfitval,nd1,nd2)
c
              jj = ntot + i
	      misfit(jj) = misfitval
              t1 = cputime(t2,t3)
              print *,'boucle i, time = ',i, t1
c
 30	   continue
           t1 = cputime(t2,t3)
           ttfor = ttfor + t2
 31        if(summary)write(*,*)' finished forward modeling at ',t1
c
...
c        write(lu_sum,*)
c        write(lu_sum,*)
c        write(lu_sum,*)'Performance statistics'
c        write(lu_sum,*)' '
c       write(lu_sum,*)
...
c
        fnme = 'na.nad'
        print *,'Appel a NA_header with nh = ',nh
	call NA_header
...
c                                               for restart part
	   if (noforward) then
		 write(lu_mis,*)misfit(i)
		 do k=1,nd
		    write(lu_mod,*)na_models(k,i)
		 enddo
		 call flush(lu_mod)
		 call flush(lu_mis)
	    endif
...
c                                               no na.nad
	   k = 1
 32	   read(lu_mis,*,end=34,err=33)misfit(k)
	   write(*,*)misfit(k)
	   do i=1,nd
 	   read(lu_mod,*,end=34,err=33)na_models(i,k)
	   enddo
c                                               end Modif sophie
