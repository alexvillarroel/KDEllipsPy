...
def _preprocess_trace(
	...
	# First trim/pad to the desired window so trace covers the target interval.
	tr.trim(starttime=starttime, endtime=endtime, pad=True, fill_value=0.0)

	# Then interpolate to the requested sampling rate. Interpolating after padding
	# avoids extrapolation errors when the original trace is shorter than desired.
	tr.interpolate(sampling_rate=1.0 / float(delta), method="linear")
...
def write_azi_times_file(
	...
	if config is not None:
		...
		if output_path is None:
			raise ValueError("output_path must be provided if config is used without input_ctl_path.")
	elif input_ctl_path is not None:
		input_ctl_path = Path(input_ctl_path).resolve()
		cfg = ConfigParser(str(input_ctl_path))
	else:
