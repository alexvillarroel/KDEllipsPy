...
subroutine reflect4 (tmin,tconv)
  ...
  integer :: ic,ics,ir,ir0,ir1,ir2,ir3,idel,is,is0,is1,is2,is3,jrs
  logical :: tmin

  logical,dimension(1:nr,1:ns) :: tconv

  complex(kind=pcal) :: egam,enu,s1phiu,s1phid
  complex(kind=pcal) :: s1psiu,s1psid,s2phiu,s2phid,s2psiu,s2psid
  complex(kind=pcal) :: s3phiu,s3phid,s3psiu,s3psid,s4phid,s4phiu,s4psid,s4psiu
  complex(kind=pcal) :: s5,s6,arg,enuinv,egaminv
  complex(kind=pcal),dimension(1:nr,1:ns,1:11) :: cdu

  real(kind=pcal) :: zc,zsc,dz,r1,i1,r2,i2

  complex(kind=pcal),dimension(1:2) :: push,pdsh
  complex(kind=pcal),dimension(1:2,1:4) :: pu,pd
  complex(kind=pcal),dimension(1:nc) :: ftdosh,ftupsh
  complex(kind=pcal),dimension(1:nc,1:2,1:2) :: ftup,ftdo

  complex(kind=pcal) :: cs2,cs3,cs4,cs5,cs6,cs7,cs8,cs9
  complex(kind=pcal) :: cr1,cr2,cr3
  complex(kind=pcal) :: cz1,cz1b,cz2,cz2b,cz3,cz3b,cz4,cz4b
  complex(kind=pcal) :: cdu1,cdu2,cdu3,cdu4,cdu5,cdu6,cdu7,cdu8,cdu9,cdu10,cdu11
  complex(kind=pcal) :: cu,cu2,cu3
  ...
  do is1 = 1,ncs			
     ...
     ! boucle sur les couches recepteur
     do ir1 = 1,ncr		
        ic = irc(ir1)
        cr1 = ai*cgam(ic)
        cr2 = cgam(ic)*cgam(ic)
        ...
        do is2 = 1,nzs(is1) 	!boucle sur les prof. sources dans ics
           ...
           do ir2 = 1,nzr(ir1)		!boucle sur les prof. recept. dans ic = irc(ir1)
              ...
              arg = -ai*cgam(ic)*zc
              if (real(arg) .lt. explim) arg = cmplx(explim,aimag(arg),pcal)
              egam = exp(arg)
              egaminv = one/egam
              arg = -ai*cnu(ic)*zc
              if (real(arg) .lt. explim) arg = cmplx(explim,aimag(arg),pcal)
              ...
              ! boucle distances radiales
              do is3 = 1,nzss(is2,is1)    
                 is = izss(is3,is2,is1)

                 ! boucle sur distances radiales
                 do ir3 = 1,nzrr(ir2,ir1)		
                    ir = izrr(ir3,ir2,ir1)
                    jrs = irs(ir,is)
                    ...
                    end if		! test conv deja obtenue
                 end do		! boucle dist. radiale
              end do		! boucle dist. radiale
           end do		! boucle prof. dans couche ic
        end do		! boucle prof. dans couche ics
     end do		! boucle sur couche ic
  end do		! boucle sur couche ics
