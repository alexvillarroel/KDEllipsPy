...
subroutine reflect2
  ...
  complex(kind=pcal) :: nbsh,mbsh
  complex(kind=pcal) :: ca1,ca2,ca3,ca4,cadet,cash,cb1,cb2,cb3,cb4,cbsh,cc1,cc2,cc3,cc4,ccsh
  complex(kind=pcal),dimension(1:2,1:2) :: nb,mb
  integer :: ic,ic1
  ...
  do ic = 1,nc-1
     ...
     ca1 = one-(rd(ic1,1,1)*nb(1,1)+rd(ic1,1,2)*nb(2,1))
     ca2 = -(rd(ic1,1,1)*nb(1,2)+rd(ic1,1,2)*nb(2,2))
     ca3 = -(rd(ic1,2,1)*nb(1,1)+rd(ic1,2,2)*nb(2,1))
     ca4 = one-(rd(ic1,2,1)*nb(1,2)+rd(ic1,2,2)*nb(2,2))
     cadet = ca1*ca4-ca2*ca3
     cash = one/(one-rdsh(ic1)*nbsh)
  ...
  do ic = nc-1,1,-1
     ...
     ic1 = ic+1
     ca1 = one-(ru(ic1,1,1)*mt(ic1,1,1)+ru(ic1,1,2)*mt(ic1,2,1))
     ca2 = -(ru(ic1,1,1)*mt(ic1,1,2)+ru(ic1,1,2)*mt(ic1,2,2))
     ca3 = -(ru(ic1,2,1)*mt(ic1,1,1)+ru(ic1,2,2)*mt(ic1,2,1))
     ca4 = one-(ru(ic1,2,1)*mt(ic1,1,2)+ru(ic1,2,2)*mt(ic1,2,2))
     cadet = ca1*ca4-ca2*ca3
     cash = one/(one-rush(ic1)*mtsh(ic1))
