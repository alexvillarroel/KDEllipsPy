...
!                                                                             *
!      Le potentiel PSI utilise pour onde SV est defini par :                 *
!                  u = rot ( rot (PSI) )                                      *
...
!      classique en divisant le potentiel PSI par one/ai/kr = coef            *
!                                                                             *
!       Ordre de stockage :                                                   *
!                              pp=(1,1)   sp=(1,2)                            *
!                              ps=(2,1)   ss=(2,2)                            *
...
subroutine reflect1
  ...
  real(kind=pcal) :: aki,rdeph,adeph

  complex(kind=pcal) :: coef,cf1,cf2,cf3,cdd,cnurho,cgarho,cnugam,cb2,ckb2i2,ckb2i1,cb1,ca1d,ca2d
  complex(kind=pcal) :: ca,cb,cc,cd,ce,cf,cg,cgkr2,ch,chkr2,cdd2
  complex(kind=pcal) :: ctmp1,ctmp2,ctmp3,ctmp4,ctmp5,ctmp6,ctmp7,ctmp8,ctmp9,ctmp10,cdeph
  complex(kind=pcal) :: cs1,cs2,cdelt

  ! Coefficient pour la convention sur PSI (coef) et sur la TF (aki=-one)

  coef = one/ai
  aki = -one

  !  CONDITIONS AUX LIMITES a la profondeur z = 0, coefficients de reflexion/transmission
  !  2 possibilites : 1) surface libre (mettre en commentaire de B1 a B2)
  !                   2) 1/2 espace sup. infini (commentaires de A1 a A2)
  ...
  do ic = 2,nc
     ...
     cb1 = cb2
     cb2 = kr2*ckb2i2
     ca1d = rho(ic1)*(one-2*cb1)
     ca2d = rho(ic)*(one-2*cb2)
     ca = ca2d-ca1d
     ...
     rdeph = real(cdeph)
     adeph = aimag(cdeph)
     if (abs(rdeph) .lt. 1.d-150) rdeph = zero
     me1(ic1) = cmplx(rdeph,adeph,pcal)

     cdeph = exp(-ai*cgam(ic1)*hc(ic1))
     rdeph = real(cdeph)
     adeph = aimag(cdeph)
     if (abs(rdeph) .lt. 1.d-150) rdeph = zero
     me2(ic1) = cmplx(rdeph,adeph,pcal)
