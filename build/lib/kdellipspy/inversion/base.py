...
class NAResult:
    ...
    def plot_yolo(self, save_path: str | Path = "dashboard.pdf",
        ...
        # Paneles aún rasterizados (se irán migrando a ejes nativos uno a uno).
        imgs = {
        ...
        # Paneles rasterizados: imshow del bitmap recortado.
        for ax, key in [(axA, "map"), (axB, "fit"),
                        (axD, "heat"), (axE, "conv"), (axF, "appr")]:
            ax.axis("off")
            if imgs[key] is not None:
                ax.imshow(imgs[key], aspect="auto")

        # Panel nativo: azimutal dibujado directo en un eje polar (sin raster).
