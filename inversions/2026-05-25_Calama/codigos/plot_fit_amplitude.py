...
def build_fig(obs, syn, time, names, azi):
    ...
    fig, axs = plt.subplots(nsta, 3, figsize=(11, 1.6*nsta),
    ...
    for j in range(nsta):
        for c in range(3):
            ...
            ax.text(0.97, 0.9, f"{pk:.2e}", transform=ax.transAxes,
    ...
    fig.text(0.005, 0.5, "Desplazamiento (m) — escala comun", rotation=90,
...
def main():
    ...
    fig.suptitle(f"Ajuste R/T/Z — amplitudes reales (eje Y comun)   "
                 f"misfit={r.best_model.misfit:.4f}", fontsize=13, y=1.005)
    plt.tight_layout()
...
def _check():
    ...
    obs = np.stack([np.array([np.sin(t), np.sin(t), np.sin(t)]),
