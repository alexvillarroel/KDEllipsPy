...
def resample_data(base_path, npts_orig, npts_new):
    ...
    files = ['real_vel_x', 'real_vel_y', 'real_vel_z', 'real_disp_x', 'real_disp_y', 'real_disp_z']
    ...
    for item in all_items:
        if item.is_file() and any(f in item.name for f in files):
            ...
            print(f"Procesado: {item.name} - Guardando {len(data_resampled)} puntos nuevos.")
            
            # 5. Guardar los datos nuevos sobreescribiendo el archivo de la carpeta principal
            np.savetxt(item, data_resampled)

if __name__ == "__main__":
    ruta_datos = '/home/alex/KDEllipsPy/inversions/2020-06-03_Calama/banda_0.02-0.1/DATA'
