...
ap = Axitra(model=model, stations=stations, sources=sources, fmax=fmax, duration=duration, latlon=True, axpath=axpath, id=124)
ap = moment.green(ap)
...
for i in range(7):
    ...
    dist_deg = dist_m / 111190.0
    arrivals_p = taup.get_travel_times(75.0, dist_deg, phase_list=["P", "p"])
    arrivals_s = taup.get_travel_times(75.0, dist_deg, phase_list=["S", "s"])
    tp_l.append(arrivals_p[0].time if arrivals_p else 10.0)
    ts_l.append(arrivals_s[0].time if arrivals_s else 25.0)

window_s = 20.0
results = []

for t0 in np.arange(0, 30.1, 0.5):
    _, sx, sy, sz = moment.conv(ap, hist, source_type=4, t0=t0, unit=2)
    syn = np.stack([sx, sy, sz], axis=1)
    syn_f = signal.filtfilt(b_filt, a_filt, syn, axis=-1)
    
    total_cc, total_num, total_den = [], 0.0, 0.0
    
    for i in range(7):
        tp, ts = tp_l[i], ts_l[i]
        kp0, kp1 = int((tp-1)*4), int((tp-1+window_s)*4)
        ks0, ks1 = int((ts-1)*4), int((ts-1+window_s)*4)
        
        # P-window: N(0) and Z(2) | S-window: E(1)
        windows = [(0, kp0, kp1), (2, kp0, kp1), (1, ks0, ks1)]
        for comp, start, end in windows:
            start, end = max(0, start), min(512, end)
            o, s = obs_f[i, comp, start:end], syn_f[i, comp, start:end]
            
            if np.std(o) > 0 and np.std(s) > 0:
                cc = np.corrcoef(o, s)[0, 1]
                total_cc.append(cc)
            
            total_num += np.sum((o - s)**2)
            total_den += np.sum(o**2)

    avg_cc = np.mean(total_cc) if total_cc else 0
    vr = (1 - (total_num / total_den)) * 100 if total_den > 0 else 0
    results.append((t0, avg_cc, vr))

best_by_cc = max(results, key=lambda x: x[1])
best_by_vr = max(results, key=lambda x: x[2])

print(f"Best by CC: t0={best_by_cc[0]:.2f}, CC={best_by_cc[1]:.4f}, VR={best_by_cc[2]:.2f}%")
print(f"Best by VR: t0={best_by_vr[0]:.2f}, CC={best_by_vr[1]:.4f}, VR={best_by_vr[2]:.2f}%")
