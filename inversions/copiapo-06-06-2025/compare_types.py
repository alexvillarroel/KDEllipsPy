...
ap = Axitra(model=model, stations=stations, sources=sources, fmax=fmax, duration=duration, latlon=True, axpath=axpath, id=126)
ap = moment.green(ap)
...
def evaluate(stype, t0):
    _, sx, sy, sz = moment.conv(ap, hist, source_type=stype, t0=t0, unit=2)
    ...
    for i in range(7):
        ...
        for comp, start, end in [(0, kp0, kp1), (2, kp0, kp1), (1, ks0, ks1)]:
            o, s = obs_f[i, comp, max(0, start):min(512, end)], syn_f[i, comp, max(0, start):min(512, end)]
            all_obs.append(o)
            all_syn.append(s)
            num += np.sum((o-s)**2)
            den += np.sum(o**2)
    ...
    misfit = num / den if den > 0 else 0
    amp = np.max(np.abs(flat_syn))
    return cc, misfit, amp

print(f"{'Type':<5} | {'t0':<5} | {'CC':<8} | {'Misfit':<8} | {'Peak Amp':<10}")
print("-" * 50)

for stype in [2, 4]:
    best_cc = -1
    best_data = None
    for t0 in np.arange(5, 20.1, 0.5):
        cc, m, a = evaluate(stype, t0)
        if cc > best_cc:
            best_cc = cc
            best_data = (t0, cc, m, a)
    print(f"{stype:<5} | {best_data[0]:<5.1f} | {best_data[1]:<8.4f} | {best_data[2]:<8.4f} | {best_data[3]:.2e}")
