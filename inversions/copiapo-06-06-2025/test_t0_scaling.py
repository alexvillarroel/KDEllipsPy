...
ap = Axitra(model=model, stations=stations, sources=sources, fmax=fmax, duration=duration, latlon=True, axpath=axpath, id=125)
ap = moment.green(ap)
...
print(f"Observed peak amplitude: {np.max(np.abs(obs)):.2e}")
...
for i in range(7):
    ...
    arr_p = taup.get_travel_times(75.0, dist_deg, phase_list=["P", "p"])
    arr_s = taup.get_travel_times(75.0, dist_deg, phase_list=["S", "s"])
    tp_l.append(arr_p[0].time if arr_p else 10.0)
    ts_l.append(arr_s[0].time if arr_s else 25.0)

window_s = 20.0
results = []

for t0 in np.arange(5, 25.1, 0.5):
    ...
    for i in range(7):
        ...
        for comp, start, end in [(0, kp0, kp1), (2, kp0, kp1), (1, ks0, ks1)]:
            all_obs.append(obs_f[i, comp, max(0, start):min(512, end)])
            all_syn.append(syn_f[i, comp, max(0, start):min(512, end)])
    
    flat_obs = np.concatenate(all_obs)
    flat_syn = np.concatenate(all_syn)
    
    if np.std(flat_syn) > 0:
        cc = np.corrcoef(flat_obs, flat_syn)[0, 1]
        # Optimal scaling: scale = sum(obs * syn) / sum(syn^2)
        scale = np.sum(flat_obs * flat_syn) / np.sum(flat_syn**2)
        scaled_syn = scale * flat_syn
        vr_opt = (1 - np.sum((flat_obs - scaled_syn)**2) / np.sum(flat_obs**2)) * 100
        results.append((t0, cc, vr_opt, scale))

best_cc = max(results, key=lambda x: x[1])
print(f"Best by CC: t0={best_cc[0]:.2f}, CC={best_cc[1]:.4f}, Opt_VR={best_cc[2]:.2f}%, Scale={best_cc[3]:.2e}")
