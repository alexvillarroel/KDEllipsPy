...
ap = Axitra(model=model, stations=stations, sources=sources, fmax=fmax, duration=duration, latlon=True, axpath=axpath, id=128)
ap = moment.green(ap)
...
misfit_raw = np.sum((fo - fs_arr)**2) / np.sum(fo**2)
scale = np.sum(fo * fs_arr) / np.sum(fs_arr**2)
scaled_syn = scale * fs_arr
misfit_scaled = np.sum((fo - scaled_syn)**2) / np.sum(fo**2)

print(f"--- Analysis for Type 4, t0=1.5, delay=6.0 ---")
print(f"Raw Misfit: {misfit_raw:.4f}")
print(f"Optimal Scale Factor: {scale:.4f}")
print(f"Misfit after scaling: {misfit_scaled:.4f} (VR = {(1-misfit_scaled)*100:.2f}%)")
print(f"Peak Obs Amp: {np.max(np.abs(fo)):.2e}")
print(f"Peak Syn Amp: {np.max(np.abs(fs_arr)):.2e}")
